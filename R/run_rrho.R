...
#' [colorRamp2][circlize::colorRamp2]). If the matrix is continuous, the value
#' can also be a vector of colors so that colors can be interpolated. Pass to
#' `ColorMapping`.  For more details and examples, please refer to
#' [ComplexHeatmap::Heatmap].
#' @param bar_col Color that maps to bottom and left bar. bar_col should be a
#' color mapping function which accepts a vector of values and returns a vector
#' of colors. Normally it is generated by [colorRamp2][circlize::colorRamp2].
...
rrho_heatmap <- function(
    rrho_obj, col = NULL, bar_col = NULL,
    name = NULL,
    row_title = NULL,
    ...
    if (is.null(bar_col)) {
        bar_col <- circlize::colorRamp2(
            c(-1L, 0L, 1L) * max(
                abs(c(column_ranked_list, row_ranked_list)),
                na.rm = TRUE
            ),
            c("blue", "#EEEEEE", "red")
        )
