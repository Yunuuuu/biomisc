test_that("estimate_ccf function works well", {
    ...
    subclonalDissection <- function(region,
                                    ...
                                    purity) {
        pyClone.tsv <- data.frame(complete.mutation.table,
        ...
        f.function <- function(c, purity, local.copy.number) {
            return(pmin(c((purity * c) / (2 * (1 - purity) + purity * local.copy.number)), 1))
        }
    ...
    data.table::setDT(region.phyloCCF2)
    data.table::setnames(
        region.phyloCCF2, c("expected.VAF", "obs.VAF", "absolute.ccf", "absolute.ccf.0.05", "absolute.ccf.0.95", "phyloCCF", "phyloCCF.0.05", "phyloCCF.0.95"), c("expVAF", "obsVAF", "absCCF", "absCCF_lower", "absCCF_higher", "phyloCCF", "phyloCCF_lower", "phyloCCF_higher")
    )
    common_names <- intersect(names(region.phyloCCF), names(region.phyloCCF2))
    testthat::expect_equal(
        region.phyloCCF[, ..common_names],
        region.phyloCCF2[, ..common_names],
